...
``\frac{\mathrm{d}N}{\mathrm{d}q} = \left( \frac{cq + q}{cq + 1} \right)^\beta``

Note the normalization, where the value is `1` at `q = 1`.
"""
function log_dNdq(q, cq, beta)
    beta*(log(cq + q) - log(cq + 1))
end
...
function make_log_dNdm1dqdVdt(alphalm1, alphamm1, alphahm1, mbreakf1, mbreaks1, alphalm2, alphamm2, alphahm2, mbreakf2, mbreaks2, alpharight2, mturn_right2, cq, beta, lambda, zp, kappa; mgrid=collect(10:0.25:100), mnorm=35.0, qnorm=1.0, znorm=0.0)
    ...
    log_qnorm = log_dNdq(qnorm, cq, beta)
    ...
    function log_dNdm1dqdVdt(m, q, z)
        ...
        log_pair_pop = log_dNdq(q, cq, beta) - log_qnorm
...
@model function pop_model_samples(evt_samples, log_prior_wts, sel_samples, log_sel_pdraw, Ndraw, m_grid, zs_interp)
    ...
    # Pop density
    log_dNdm1dqdVdt = make_log_dNdm1dqdVdt(alphalm1, alphamm1, alphahm1, mbreakf1, mbreaks1, alphalm2, alphamm2, alphahm2, mbreakf2, mbreaks2, alpharight2, mturn_right2, cq, beta, lambda, zp, kappa; mgrid=m_grid)
