...
function make_log_dNdm_peak_gridded(mgrid, mu_peak, sigma_peak)
    ms = mgrid

    log_dNdms = logpdf(Normal(mu_peak, sigma_peak), ms)
...
function log_dNdq(q, mu, sigma, f, cq, beta)
    if !(0.0 <= q <= 1.0) || !(sigma > 0.0) || !(0.0 <= f <= 1.0)
        return -Inf
    end
    ...
    d = truncated(Normal(mu, sigma), 0.0, 1.0)

    logNq = logpdf(d, q)

    logN1 = logpdf(d, 1.0)  # finite unless extremely tiny; will propagate as -Inf if needed

    a = (f == 0.0) ? -Inf : (log(f) + (logNq - logN1))
    b = (f == 1.0) ? -Inf : (log1p(-f) + beta * (log(cq+q) - log1p(cq)))
...
Left (m ≤ mturn_right):
  3-segment broken power law, normalized on [mlow, mturn_right]

Right (m > mturn_right):
  Single power law tail, continuous at mturn_right

Left segments (continuous at both breaks):
p(m) ∝ (m/mbreakf1)^(-alphalm1)                                    for m ∈ [mlow, mbreakf1]
p(m) ∝ (m/mbreakf1)^(-alphamm1)                                    for m ∈ (mbreakf1, mbreaks1]
p(m) ∝ (mbreaks1/mbreakf1)^(-alphamm1) * (m/mbreaks1)^(-alphahm1)   for m ∈ (mbreaks1, mturn_right]
"""
function make_combined_log_dNdm(alphalm1, alphamm1, alphahm1, mbreakf1, mbreaks1, alpharight, mturn_right; mlow = 5.0)
    @assert mlow < mbreakf1 < mbreaks1 < mturn_right "Require mlow < mbreakf1 < mbreaks1 < mturn_right."
    ...
    # continuity factor at second break so middle and high match at mbreaks1
    C    = (mbreaks1 / mbreakf1)^(-alphamm1)
    logC = -alphamm1 * (log(mbreaks1) - log(mbreakf1))

    # normalization on [mlow, mturn_right]
    Z1 = integrate_segment(mlow,        mbreakf1,    alphalm1, mbreakf1)
    Z2 = integrate_segment(mbreakf1,    mbreaks1,    alphamm1, mbreakf1)
    Z3 = C * integrate_segment(mbreaks1, mturn_right, alphahm1, mbreaks1)
    ...
    # Continuity anchor at mturn_right
    log_at_turn = log_dN_left(mturn_right)

    # Right PL, normalized at mturn_right via the anchor above
    function log_dN_right(m::Real)
        return log_at_turn + alpharight * (log(m) - lmt)
    end

    # Final: piecewise with continuity at mturn_right
...
function make_log_dNdm1dqdVdt(alphalm1, alphamm1, alphahm1, mbreakf1, mbreaks1, alphalm2, alphamm2, alphahm2, mbreakf2, mbreaks2, alpharight2, mturn_right2, mu, sigma, f, cq, beta, lambda, zp, kappa; mgrid=collect(10:0.25:100), mnorm=35.0, qnorm=1.0, znorm=0.0)
    log_dNdm_total1 = make_combined_log_dNdm1(alphalm1, alphamm1, alphahm1, mbreakf1, mbreaks1)
    log_dNdm_total2 = make_combined_log_dNdm(alphalm2, alphamm2, alphahm2, mbreakf2, mbreaks2, alpharight2, mturn_right2)
    ...
    log_qnorm = log_dNdq(qnorm, mu, sigma, f, cq, beta)
    ...
    function log_dNdm1dqdVdt(m, q, z)
        ...
        log_pair_pop = log_dNdq(q, mu, sigma, f, cq, beta) - log_qnorm
...
@model function pop_model_samples(evt_samples, log_prior_wts, sel_samples, log_sel_pdraw, Ndraw, m_grid, zs_interp)
    ...
    mbreakf1 ~ Uniform(20, 26)
    mbreaks1 ~ Uniform(30, 40)
    
    alphalm2 ~ Uniform(-10, 10)
    alphamm2 ~ Uniform(-50, 10)
    alphahm2 ~ Uniform(-10, 10)
    mbreakf2 ~ Uniform(3, 24)
    mbreaks2 ~ Uniform(28, 35)

    alpharight2 ~ Uniform(-20, 10)
    mturn_right2 ~ Uniform(36, 60)

    mu ~ Uniform(0.1, 1.0)
    sigma ~ Uniform(0.1, 1.0)
    f ~ Uniform(0, 1)
    cq ~ Uniform(0.1, 50)
    beta ~ Uniform(-50, 10)
    ...
    # Pop density
    log_dNdm1dqdVdt = make_log_dNdm1dqdVdt(alphalm1, alphamm1, alphahm1, mbreakf1, mbreaks1, alphalm2, alphamm2, alphahm2, mbreakf2, mbreaks2, alpharight2, mturn_right2, mu, sigma, f, cq, beta, lambda, zp, kappa; mgrid=m_grid)
    ...
    (Neff_sel = model_genq.Neff_sel, R = model_genq.R, Neff_samps = model_genq.Neff_samps, m1s = m1s, m2s = m2s, qs = qs, zs = zs, m1_draw = m1_draw, m2_draw = m1_draw * q_draw, q_draw = q_draw, z_draw = z_draw)
end
