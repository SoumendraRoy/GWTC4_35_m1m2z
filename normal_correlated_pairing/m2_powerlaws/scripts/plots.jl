...
population_params = [:R, :alphalm1, :alphamm1, :alphahm1, :mbreakf1, :mbreaks1, :alphalm2, :alphamm2, :alphahm2, :mbreakf2, :mbreaks2, :alpharight2, :mturn_right2, :mu, :sigma, :f, :cq, :beta, :lambda, :zp, :kappa, :Neff_sel]
cosmo_params = [:h, :Ω_M]
...
function log_dN_from_chain_sample(trace, draw, chain)
    ...
    log_dN = BumpCosmologyGWTC3.make_log_dNdm1dqdVdt(p.alphalm1[draw=d, chain=c], p.alphamm1[draw=d, chain=c], p.alphahm1[draw=d, chain=c], p.mbreakf1[draw=d, chain=c], p.mbreaks1[draw=d, chain=c], p.alphalm2[draw=d, chain=c], p.alphamm2[draw=d, chain=c], p.alphahm2[draw=d, chain=c], p.mbreakf2[draw=d, chain=c], p.mbreaks2[draw=d, chain=c], p.alpharight2[draw=d, chain=c], p.mturn_right2[draw=d, chain=c], p.mu[draw=d, chain=c], p.sigma[draw=d, chain=c], p.f[draw=d, chain=c], p.cq[draw=d, chain=c], p.beta[draw=d, chain=c], p.lambda[draw=d, chain=c], p.zp[draw=d, chain=c], p.kappa[draw=d, chain=c])
    log_dN
end
